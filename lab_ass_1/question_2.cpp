...
private:
  ...
  bool operator>(Student object) {
    ...
    return ((*scores + *(scores + 1) + *(scores + 2)) / 3 >
            (*object.scores + *(object.scores + 1) + *(object.scores + 2)) / 3);
  }
...
void operator>>(void *cin, Student &object) { return (object.inputData()); }

void operator<<(void *cout, Student &object) { return (object.showInfo()); }
...
void c() {
  ...
  while (fgets(all_information_from_a_file, 29, a_file) != NULL &&
         student_object < 10) {
    ...
    for (unsigned char value = 0; value < 3; ++value) {
      ...
      sscanf(information_from_a_file, "%Lf",
             (*(student_objects + student_object)).scores + value);
    }
    if ((*(*(student_objects + student_object)).scores +
         *(*(student_objects + student_object)).scores + 1 +
         *(*(student_objects + student_object)).scores + 2) /
            3.0 >
        highest_average_score) {
      highest_average_score =
          (*(*(student_objects + student_object)).scores +
           *(*(student_objects + student_object)).scores + 1 +
           *(*(student_objects + student_object)).scores + 2) /
          3.0;
  ...
  printf(
      "%s\n%Lf %Lf %Lf\n",
      (*(student_objects + the_student_with_highest_average_score)).name.data(),
      *((*(student_objects + the_student_with_highest_average_score)).scores),
      *(((*(student_objects + the_student_with_highest_average_score)).scores) +
        1),
      *(((*(student_objects + the_student_with_highest_average_score)).scores) +
        2));
...
int main(int argc, char **argv) {
  ...
  while (getchar() != '\n')
    ;
  cout << object;
  printf("Return true if average score of object1 is larger than that of "
         "object2; return false otherwise: %hhu\n",
         an_object > object);
  puts("Return a result object that has the same name, but all scores are "
       "increased:");
  printf("%s\n%Lf %Lf %Lf\n", ((int)'n' + an_object).name.data(),
         *(((int)'n' + an_object).scores), *(((int)'n' + an_object).scores + 1),
         *(((int)'n' + an_object).scores + 2));
  c();
