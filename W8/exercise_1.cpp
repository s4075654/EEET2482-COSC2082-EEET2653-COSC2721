...
public:
  ...
  void doReturn(Book &abook) {
    ...
    borrowedBooks.erase(std::remove(borrowedBooks.begin(), borrowedBooks.end(), &abook), borrowedBooks.end());
    abook.namesOfBorrowers.erase(std::remove(abook.namesOfBorrowers.begin(), abook.namesOfBorrowers.end(), abook.title), abook.namesOfBorrowers.end());
    ++abook.availableCopies;
  }
};
